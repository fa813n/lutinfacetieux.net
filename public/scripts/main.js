...
function displayCell() {
	...
	if (currentCellDate > today) {
		window.alert('Tu es bien pressé! Cette case révelera son contenu en temps et en heure...');
		gameZone.setAttribute('class', 'no-display');
		answerZone.innerHTML = '<h1>Rien à voir içi!</h1><h2>en tout cas... pour le moment...</h2>';
	}
...
function displayAdditionalForm() {
	...
	const choosenGame = document.getElementById('choosen-game').value;

	const gameConstructorObject = JSON.parse(gameConstructor);
	const parameters = gameConstructorObject[choosenGame]['parameters'];
